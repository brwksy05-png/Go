...
bot.onText(/\/start/, (msg) => {
  bot.sendMessage(msg.chat.id, "مرحباً يا بطل! 🚀\n\n✉️ أرسل الإيميل الآن لإرسال رابط التسجيل مباشرة (اتصال مباشر).");
});
...
bot.on('message', async (msg) => {
  ...
  const statusMsg = await bot.sendMessage(chatId, "⏳ جاري إرسال الطلب باتصال مباشر...");
  ...
  try {
    ...
    if (!freshFlwssn) {
      return bot.editMessageText("❌ السيرفر لم يرجع تذكرة flwssn (قد يكون الـ IP محظور بالكامل).", {
        chat_id: chatId,
    ...
    // 2. تهيئة عملية التسجيل (InitSignup)
    const h1 = {
      ...reqHeaders,
    ...
    if (!state1) {
      return bot.editMessageText(`❌ فشل في الخطوة 1:\n\`\`\`json\n${JSON.stringify(res1.data).slice(0, 250)}\n\`\`\``, {
        chat_id: chatId,
        message_id: statusMsg.message_id,
        parse_mode: "Markdown"
      });
    ...
    if (!state2) {
      return bot.editMessageText(`❌ فشل في الخطوة 2:\n\`\`\`json\n${JSON.stringify(res2.data).slice(0, 250)}\n\`\`\``, {
        chat_id: chatId,
        message_id: statusMsg.message_id,
        parse_mode: "Markdown"
      });
    ...
    if (resRaw.includes('CLCSScreenUpdateTransition') || resRaw.includes('emailRegisterLinkSent')) {
      bot.editMessageText(`🎉 **تم طلب الإرسال المباشر!**\n\n📧 الإيميل: \`${email}\`\n\n📌 الرد النهائي:\n\`\`\`json\n${resRaw.slice(0, 250)}\n\`\`\``, {
        chat_id: chatId,
        message_id: statusMsg.message_id,
        parse_mode: "Markdown"
      });
    ...
    bot.editMessageText(`❌ خطأ أثناء الاتصال:\n${error.message}`, {
      chat_id: chatId,
      message_id: statusMsg.message_id
    });
  }
